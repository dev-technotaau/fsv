"""
SegFormer B0 Fine-Tuning for Fence Detection - v4.0 GPU OPTIMIZED
===================================================================
ALL OPTIMIZATIONS APPLIED:
- Fixed interpolation dimension handling
- Proper upsampling in loss and metrics
- Correct tensor shapes throughout
- Robust error handling
- Mixed precision training (AMP)
- Optimized data loading pipeline
- GPU memory preallocation
- Smooth GPU utilization (6 workers, prefetch=3)
- TensorFloat-32 matmul precision
- Reduced CPU-heavy augmentations
...
class Config:
    ...
    # Training
    INPUT_SIZE = 512
    BATCH_SIZE = 6  # Optimized for 6GB Laptop GPU (safe batch size)
    ACCUMULATION_STEPS = 2  # Effective batch size = 12 (6 x 2)
    EPOCHS = 50
    LEARNING_RATE = 1e-4
    WEIGHT_DECAY = 0.01
    
    # Hardware - GPU Optimizations
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    NUM_WORKERS = 4  # Balanced for 6GB GPU (more = more RAM usage)
...
def get_augmentation(train=True):
    ...
    # Optimized augmentations - removed CPU-heavy ones for smoother pipeline
    return A.Compose([
        A.HorizontalFlip(p=0.5),
        A.Rotate(limit=10, p=0.3),  # Reduced rotation for speed
