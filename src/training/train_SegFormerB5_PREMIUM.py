...
class Config:
    ...
    # Training - Enhanced for SegFormer-B5 (Optimized for 6GB GPU)
    INPUT_SIZE = 512  # Balanced for 6GB GPU (640 requires 8GB+)
    ...
    KEEP_LAST_N_CHECKPOINTS = 5  # Keep only last N checkpoints to save space
    
    # Hardware - GPU Optimizations for B5 (6GB GPU)
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    NUM_WORKERS = 2  # Reduced for 6GB GPU
    PIN_MEMORY = True
    PREFETCH_FACTOR = 2  # Reduced for 6GB GPU
...
def train_segformer_b5():
    ...
    logger.info("="*70)
    logger.info("SEGFORMER-B5 PREMIUM TRAINING v5.0")
    logger.info("="*70)
    ...
    # Warm up GPU with aggressive memory management for 6GB VRAM
    if torch.cuda.is_available():
        logger.info("Warming up GPU...")
        torch.cuda.empty_cache()
        # Use single sample for warmup to minimize memory
        ...
        del dummy_input
        torch.cuda.empty_cache()
        logger.info(f"GPU ready - Free VRAM: {torch.cuda.mem_get_info()[0] / 1024**3:.2f} GB")
    ...
    # Final summary
    logger.info("\n" + "="*70)
    logger.info("🎉 TRAINING COMPLETE - SEGFORMER-B5 PREMIUM 🎉")
    logger.info("="*70)
    ...
    logger.info(f"   Input Resolution:          {Config.INPUT_SIZE}x{Config.INPUT_SIZE}")
    logger.info(f"")
    ...
    logger.info(f"   1. Export to ONNX: python export_segformer_to_onnx.py")
    logger.info(f"   2. Test on web: Open index_segformer_web.html")
    logger.info(f"   3. Run inference: python inference_segformer.py")
    logger.info("="*70)
